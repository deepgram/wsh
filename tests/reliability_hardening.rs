...
//! - Phase 5: Input capture ownership enforcement via HTTP
...
// ── Phase 5: Input Capture Ownership via HTTP ───────────────────────────────

#[tokio::test]
async fn test_input_capture_owner_conflict_http() {
    let (state, _, _) = common::create_test_state();
    let app = router(state, None);

    // Agent 1 captures input -- should succeed (204)
    let response = app
        .clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/sessions/test/input/capture?owner=agent-1")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_eq!(
        ...
        "agent-1 capture should succeed"
    );

    // Agent 2 tries to capture -- should fail (409 Conflict)
    let response = app
        .clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/sessions/test/input/capture?owner=agent-2")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_eq!(
        response.status(),
        StatusCode::CONFLICT,
        "agent-2 capture should be rejected"
    );
    ...
    let body = axum::body::to_bytes(response.into_body(), usize::MAX)
        ...
        .unwrap();
    let json: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(json["error"]["code"], "input_capture_failed");

    // Agent 2 tries to release -- should fail (409 Conflict, not the owner)
    let response = app
        .clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/sessions/test/input/release?owner=agent-2")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_eq!(
        response.status(),
        StatusCode::CONFLICT,
        "agent-2 release should be rejected"
    );

    // Agent 1 releases -- should succeed (204)
    let response = app
        .clone()
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/sessions/test/input/release?owner=agent-1")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_eq!(
        response.status(),
        StatusCode::NO_CONTENT,
        "agent-1 release should succeed"
    );
...
#[tokio::test]
async fn test_input_capture_same_owner_idempotent() {
    let (state, _, _) = common::create_test_state();
    let app = router(state, None);

    // Same owner can capture multiple times (idempotent)
    for _ in 0..3 {
        let response = app
            .clone()
            .oneshot(
                Request::builder()
                    .method("POST")
                    .uri("/sessions/test/input/capture?owner=same-agent")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/sessions/test/input/release?owner=same-agent")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn test_socket_initial_frame_timeout() {
    ...
    tokio::spawn(async move {
        wsh::server::serve(sessions, &path_clone, cancel_clone)
            .await
            .unwrap();
    });
